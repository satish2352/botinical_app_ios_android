...
const PriortyArvrdetails = ({ route }) => {
    const navigation = useNavigation();
    const {data,idArray} = route.params;
    const [audioModalVisible, setAudioModalVisible] = useState(false);
    const [videoModalVisible, setvideoModalVisible] = useState(false);
    const { SelectedLanguage1 } = globalvariavle();
    const [about, setaboutData] = useState([]);
    const [currentid, setCurrentid] = useState(data.order_number);
    ...
    const handleNext = () => {
        if (currentIndex < idArray.length - 1) {
          ...
          setCurrentid(idArray[nextIndex]);
        }
      };
      ...
      const handleBack = () => {
        if (currentIndex > 0) {
          ...
          setCurrentid(idArray[prevIndex]);
        }
      };
    const carouselData = [
...
export default PriortyArvrdetails;
